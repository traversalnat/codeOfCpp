...
class complex {
 ...
 public:
    ...
    explicit complex(double r) : re{r}, im{0} {}  // 支持 complex a {2.3}的操作
...
complex operator/(complex a, int b) { return a; }
...
int main() {
    complex a{2.3};
    complex b{a / 0};  // 这里存在隐式转换，实数0
                       // 转换为complex类型(要求complex类存在该构造方法)
...
class Vector {
 ...
 public:
    ...
    void push_back(double x) {
        // elem 中有 maxsz (sz * 2) 个数据，直到用完再重新分配
        if (sz >= maxsz) {
            maxsz *= 2;
            double* p = new double[maxsz]();
            for (int i = 0; i < sz; i++) {
                p[i] = elem[i];
            }
            p[sz] = x;
        ...
        /*
         * realloc 对 new 分配的内存空间结果随机
        if (sz >= maxsz) {
            maxsz *= 2;
            // std::realloc 要求内存区域是由std::malloc, std::calloc,
        std::realloc分配的 elem = (double *)std::realloc(elem, maxsz);
        }
        elem[sz++] = x;
        */
    }
    ...
    */
    const double* begin() { return sz ? &elem[0] : nullptr; }

    const double* end() { return begin() + sz; }
};
...
int main() {
    ...
    for (int i = 0; i < 5; i++) vc.push_back(i + 5.21);
    for (auto x : vc) {
        std::cout << x << " ";
    }
    std::cout << std::endl;
...
class Vector_container : public Container {
 ...
 public:
    ...
    // 自定义成员函数
    void swink() { std::cout << "Vector_container.swink\n"; }
};
...
class Vector {
 ...
 public:
    ...
    // 拷贝构造函数
    Vector(const Vector& x) : elem{new double[x.sz]}, sz{x.sz} {
